...
interface InstallmentRow {
    ...
    total: number;
    status: string;
    raw_ap: AccountPayable;
    raw_inst: any;
}

// Flat-mapping accountsPayable.data into individual installments
const installmentsList = computed<InstallmentRow[]>(() => {
    const list: InstallmentRow[] = [];
    if (!props.accountsPayable?.data) return list;

    props.accountsPayable.data.forEach((ap) => {
        const insts = ap.installments || [];
        if (insts && insts.length > 0) {
            insts.forEach((inst) => {
                list.push({
                    ...
                    bank_account_name:
                        ap.bank_account?.name || props.bankAccount?.name || "-",
                    due_date: inst.due_date,
                    payment_date: inst.payment_date || null,
                    installment_number: inst.installment_number,
                    total_installments: ap.total_installments || 1,
            ...
            list.push({
                ...
                bank_account_name:
                    ap.bank_account?.name || props.bankAccount?.name || "-",
                due_date: ap.due_date || ap.created_at || "",
                payment_date: null,
...
function reload(extraParams = {}) {
    ...
    router.get(route("tenant.finance.accounts-payable.list"), params, {
        preserveState: true,
        preserveScroll: true,
    });
}
...
function executeDelete() {
    if (itemToDelete.value) {
        router.delete(
            route(
                "tenant.finance.accounts-payable.destroy",
                itemToDelete.value,
            ),
            {
                preserveScroll: true,
...
<template>
    ...
    <div class="space-y-6">
        ...
        >
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    Gerencie seus lançamentos e obrigações financeiras.
                </p>
            </div>
            <Button
                v-if="permissions.includes('finance.accounts_payable.create')"
        ...
        >
            ...
            <div
                v-if="props.accountsPayable?.last_page > 1"
            ...
            >
                <div class="text-sm text-muted-foreground">
                    ...
                    {{ props.accountsPayable.total }} total
                </div>
                <div class="flex items-center gap-1.5">
                    <Button
                        v-for="(link, lIdx) in props.accountsPayable.links"
                        :key="lIdx"
                        variant="outline"
                        size="sm"
                        :class="[
        ...
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    ...
                    permanentemente o lançamento de contas a pagar e todas as
