...
const form = useForm({
    financial_category_id: props.accountPayable.financial_category_id ? String(props.accountPayable.financial_category_id) : "",
    financial_subcategory_id: props.accountPayable.financial_subcategory_id ? String(props.accountPayable.financial_subcategory_id) : "",
    cost_id: props.accountPayable.cost_id ? String(props.accountPayable.cost_id) : "",
    bank_account_id: props.accountPayable.bank_account_id ? String(props.accountPayable.bank_account_id) : "",
    financial_contact_id: props.accountPayable.financial_contact_id ? String(props.accountPayable.financial_contact_id) : "",
    ...
    value: props.accountPayable.installments?.[0]?.value !== undefined
        ? maskCurrency(String(props.accountPayable.installments[0].value))
        : (props.accountPayable.total !== undefined ? maskCurrency(String(props.accountPayable.total)) : ""),
    due_date: props.accountPayable.installments?.[0]?.due_date
        ? props.accountPayable.installments[0].due_date.split("T")[0]
        : (props.accountPayable.due_date ? props.accountPayable.due_date.split("T")[0] : ""),
    status: props.accountPayable.installments?.[0]?.status || props.accountPayable.status || "open",
...
function submit() {
    ...
    form.transform(() => payload).put(route("tenant.finance.accounts-payable.update", props.accountPayable.id));
}
</script>
...
<template>
    ...
    <div class="mb-6 flex flex-col gap-4 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-muted-foreground">Atualize as informações deste lançamento de contas a pagar.</p>
        </div>
