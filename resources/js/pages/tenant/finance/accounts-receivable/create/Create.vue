...
const submitUrl = ref(route("tenant.finance.accounts-receivable.store"));

onMounted(() => {
    if (window.location.search) {
        backUrl.value = route("tenant.finance.accounts-receivable.list") + window.location.search;
        submitUrl.value = route("tenant.finance.accounts-receivable.store") + window.location.search;
    }
});
...
<template>
    <Head title="Novo Lançamento - Contas a Receber" />
    ...
    <div class="mb-6 flex flex-col gap-4 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-muted-foreground">Registre um novo contas a receber no financeiro.</p>
        </div>
