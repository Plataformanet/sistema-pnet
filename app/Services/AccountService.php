...
abstract class AccountService
{
    ...
    public function findAll($request, string $periodo)
    {
        ...
        return $this->model::when($request->query('conta_id') !== null, function (Builder $query) use ($request) {
            $query->whereHas('accountBank', function (Builder $query) use ($request) {
                $query->where('account_bank_id', $request->query('conta_id'));
            });
        })
    ...
    public function updateInstallment(string $id): bool
    {
        return DB::transaction(function () use ($id) {

            $installment = Installment::findOrFail($id);

            $model = $installment->parcelable_type;

            $bankAccountId = $model::select('account_bank_id')->where('id', $installment->parcelable_id)->first()->account_bank_id;

            $bankAccount = AccountBank::findOrFail($bankAccountId);

            if (class_basename($model) === 'AccountsPayable') {
                $bankAccount->current_balance -= $installment->value;
                $bankAccount->save();
            }

            if (class_basename($model) === 'AccountsReceivable') {
                $bankAccount->current_balance += $installment->value;
                $bankAccount->save();
            }
    ...
    public function totalPeriod($request, string $period, ?int $bankAccountId = null)
    {
        ...
        })->whereHasMorph('parcelable', [$this->model], function (Builder $query) use ($start, $end, $request, $bankAccountId) {
            $query->whereBetween('due_date', [$start, $end])
                ->when($request->query('categoria_id'), function (Builder $query) use ($request) {
                    $query->where('category_financial_id', $request->query('categoria_id'));
                })
                ...
                ->when($bankAccountId !== null, function (Builder $query) use ($bankAccountId) {
                    $query->where('account_bank_id', $bankAccountId);
                });
        });
    ...
    public function totalPaid($request, string $periodo, ?int $bankAccountId = null)
    {
        $start = $request->query('inicio')
            ? Carbon::parse($request->query('inicio'))->startOfDay()
            : Carbon::createFromFormat('Y-m', $periodo)->startOfMonth();
    ...
    public function totalToDue($request, int $days, string $period, ?int $bankAccountId = null)
    {
        ...
        })->whereHasMorph('parcelable', [$this->model], function (Builder $query) use ($start, $end, $statusOpen, $request, $bankAccountId) {
    ...
    public function totalDueToday($request, string $period, ?int $bankAccountId = null)
    {
        $statusOpen = AccountsEnum::OPEN->value;
        $today      = now()->startOfDay();
        ...
        })->whereHasMorph('parcelable', [$this->model], function (Builder $query) use ($today, $statusOpen, $request, $bankAccountId) {
    ...
    public function totalOverdue($request, string $period, ?int $bankAccountId = null)
    {
        $statusOpen = AccountsEnum::OPEN->value;
        $today      = Carbon::today();
        ...
        $query = Installment::where(function (Builder $query) use ($request) {
            ...
            $query->when(is_numeric($search), function (Builder $query) use ($search) {
                $valor  = (float) $search;
                $margem = 100;
                $query->orWhereBetween('valor', [$valor - $margem, $valor + $margem]);
            });
        })->whereHasMorph('parcelable', [$this->model], function (Builder $query) use ($start, $end, $today, $statusOpen, $request, $bankAccountId) {
            $query->whereBetween('due_date', [$start, $end])
    ...
    public function updateInstallmentValue(array $data)
    {
    ...
    public function search($request, string $period)
    {
        $start = $request->query('inicio')
            ? Carbon::parse($request->query('inicio'))->startOfDay()
            : Carbon::createFromFormat('Y-m', $period)->startOfMonth();
